...
import { blitIcon } from "./primitives.js";
export async function renderIcons(atlas, layout, missing) {
    // Controller grid
    ...
    for (const [ci, col] of DIAG_COLS.entries()) {
        ...
        if (srcPath)
            await blitIcon(atlas, srcPath, layout.diagIconX, layout.diagIconY(ci), DIAG_CELL, missing);
    }
}
