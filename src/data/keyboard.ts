...
import type { KeyIcon } from "../types.js";
import { DIR } from "../config.js";

const p = (dir: string, file: string): string => path.join(dir, file);
const k = (file: string, label: string): KeyIcon => ({ file: p(DIR.kb, file), label });

export const KEYBOARD_ICONS: readonly KeyIcon[] = [
...
export const KEYBOARD_ICONS_LIGHT: readonly KeyIcon[] = KEYBOARD_ICONS.map(({ file, label }) => ({
